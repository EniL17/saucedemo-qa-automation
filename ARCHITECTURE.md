...
**Investigation process:** confirmed the pattern was WebKit-specific and full-suite-specific (not reproducible when the affected file was run alone) — evidence of CPU resource contention across simultaneously-active browser instances, not an application defect.Three mitigations were applied: extending the explicit visibility timeout, verifying the menu container's render (not just one child link) before interacting with it, and configuring file-level retries so a transient timing failure gets two further attempts rather than failing immediately. Together these bring the flake rate to effectively zero under normal runs, while remaining honest that the underlying cause is environmental resource contention rather than an application or test-logic defect.
